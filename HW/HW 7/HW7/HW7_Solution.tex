...
\begin{enumerate}
    ...
    The optimal \(k\)-step value function satisfies:
    ...
    \]
    Given \(R(s,a) \geq 0\), we prove by induction:
    \begin{itemize}
        \item Base case (\(k=0\)): \(V_0^*(s) = 0 \leq \frac{R_{\text{max}}}{1-\gamma}\)
        \item Inductive step: Assume \(V_{k-1}^*(s) \leq \frac{R_{\text{max}}}{1-\gamma}\), then
        \[
        V_k^*(s) \leq R_{\text{max}} + \gamma \cdot \frac{R_{\text{max}}}{1-\gamma} = \frac{R_{\text{max}}}{1-\gamma}
        \]
    \end{itemize}
    Thus, \(V_k^*(s) \leq \frac{R_{\text{max}}}{1-\gamma}\) for all \(k,s\). \hfill \(\square\)
    ...
    For any \(k\), define policy \(\pi\) following the \(k\)-step optimal policy for first \(k\) steps:
    \[
    V_{k+1}^\pi(s) = R(s,\pi(s)) + \gamma \sum_{s'} P(s'|s,\pi(s)) V_k^*(s') \geq V_k^*(s)
    \]
    By Monotone Convergence Theorem, \(V_k^* \to V^*\) satisfying \(V^* = BV^*\). \hfill \(\square\)
    ...
    \]
    By Banach's Fixed-Point Theorem, \(V^*\) is unique and optimal. \hfill \(\square\)
...
\]
  ...
  % Derive the optimal value function \( \hat{V}^* \) in terms of the original \( V^* \) and \( r_0 \).
        ...
        For \(\hat{R}(s,a) = R(s,a) + r_0\):
        \[
        \hat{V}_k^*(s) = V_k^*(s) + \frac{r_0}{1-\gamma}
        \]
        The Bellman equation becomes:
        \[
        \hat{V}_k^*(s) = \max_a \left[ R(s,a) + r_0 + \gamma \sum_{s'} P(s'|s,a) \hat{V}_{k-1}^*(s') \right]
        \]
        Subtracting \(\frac{r_0}{1-\gamma}\) recovers original Bellman equation. \hfill \(\square\)
    ...
    Counterexample: If \(s_{\text{end}}\) exists with \(R(s_{\text{end}},a) = 0\), adding \(r_0 > 0\) makes agents avoid termination to collect \(r_0\) indefinitely. \hfill \(\square\)
...
\item Let \( \pi_k \) be the policy at iteration \( k \). Prove the following:
    ...
    For greedy policy \(\pi_{k+1}\):
    \[
    V^{\pi_{k+1}}(s) \geq Q^{\pi_k}(s,\pi_{k+1}(s)) \geq V^{\pi_k}(s)
    \]
    Strict inequality holds unless \(\pi_k\) is optimal. \hfill \(\square\)
    ...
    Finite MDPs have \(|\mathcal{A}|^{|\mathcal{S}|}\) deterministic policies. Each iteration strictly improves \(V^\pi\) until optimal. \hfill \(\square\)
...
\item Prove that Value Iteration and Policy Iteration both converge to the same optimal value function \( V^* \), even if the policies may differ. How the policies are still optimal despite possible differences?
    ...
    Both algorithms solve \(V = BV\), which has unique fixed point \(V^*\). Optimal policies may differ but satisfy \(V^\pi = V^*\). \hfill \(\square\)
    ...
    \begin{itemize}
    \item Policy Iteration: \(O(|\mathcal{S}|^3)\) (matrix inversion) + \(O(|\mathcal{S}|^2|\mathcal{A}|)\) (improvement)
    ...
    \begin{itemize}
    \item Value Iteration: Diverges for infinite-horizon MDPs
    \item Policy Iteration: Fails as \(V^\pi\) becomes unbounded
    \end{itemize}
    Require \(\gamma < 1\) for convergence. \hfill \(\square\)
...
\begin{enumerate}[]
    ...
    For any policy \(\pi\), \(B^\pi\) is a \(\gamma\)-contraction in \(\|\cdot\|_\infty\). \\
    \textit{Proof:} Let \(\delta = \|V - V'\|_\infty\). For any \(s\):
    \[
    |B^\pi V(s) - B^\pi V'(s)| \leq \gamma \sum_{s'} P(s'|s,\pi(s)) |V(s') - V'(s')| \leq \gamma \delta
    \]
    Taking max over \(s\) gives \(\|B^\pi V - B^\pi V'\|_\infty \leq \gamma \|V - V'\|_\infty\). \(\square\)
    ...
    Banach Fixed-Point Theorem guarantees unique fixed point for contractions. \\
    \textit{Proof:} From (1), \(B^\pi\) is a contraction. Since \(V^\pi = B^\pi V^\pi\) exists, uniqueness follows. \(\square\)
    ...
    Linear operators preserve elementwise inequalities. \\
    \textit{Proof:} For all \(s\):
    \[
    B^\pi V(s) = r(s,\pi(s)) + \gamma \sum_{s'} P(s'|s,\pi(s))V(s') \leq r(s,\pi(s)) + \gamma \sum_{s'} P(s'|s,\pi(s))V'(s') = B^\pi V'(s)
    \]
    \(\square\)
...
\begin{enumerate}[resume*]
    ...
    \(\|BV - V\| = 0 \iff V = V^*\). Follows from Bellman optimality: \(V^* = BV^*\). \(\square\)
    ...
    Triangle inequality and contraction properties. \\
    \textit{Proof:} Let \(\delta = \|V - B^\pi V\|\). Then:
    \[
    \|V - V^\pi\| \leq \sum_{k=0}^\infty \gamma^k \|B^\pi V - V\| = \frac{\delta}{1-\gamma}
    \]
    Similarly for \(V^*\) using \(B\)'s contraction. \(\square\)
...
\begin{enumerate}[resume*]
    ...
    From (5), \(\|V - V^*\| \leq \frac{\varepsilon}{1-\gamma}\). Then:
    \[
    V^\pi(s) \geq V(s) - \frac{\varepsilon}{1-\gamma} \geq V^*(s) - \frac{2\varepsilon}{1-\gamma}
    \]
    \(\square\)
    ...
    Medical treatment planning - guarantees policy performance stays within safe bounds relative to optimal.
    ...
    No. Different \(V, V'\) can yield \(\pi \neq \pi'\) while having same \(\varepsilon\). Example: \(V = V^* + \epsilon\), \(V' = V^* - \epsilon\).
...
\begin{enumerate}[resume*]
    ...
    \textit{Proof:} From \(V^\pi \leq V^* \leq V\) and (5):
    \[
...
Hint: Try to apply induction. What is $\lim_{n \to \infty} B^nV$?
    ...
    \textit{Proof:} By induction, \(B^n V \leq V\). Since \(\lim_{n\to\infty} B^n V = V^*\), we get \(V^* \leq V\). \(\square\)
    ...
    \textit{Proof Sketch:} Iterate Bellman equation:
    \[
    V^\pi \geq B^\pi V - \gamma\varepsilon\mathbf{1} \geq V^* - \frac{\gamma\varepsilon}{1-\gamma}\mathbf{1}
    \]
    where \(\mathbf{1}\) is all-ones vector. \(\square\)
