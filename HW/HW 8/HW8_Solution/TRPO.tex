...
\paragraph{a) Proof of Equation (\ref{eq_3})}
\begin{align*}
\eta(\pi') - \eta(\pi) &= \mathbb{E}_{\tau \sim \pi'}\left[\sum_{t=0}^\infty \gamma^t r(s_t)\right] - \mathbb{E}_{\tau \sim \pi}\left[\sum_{t=0}^\infty \gamma^t r(s_t)\right] \\
&= \mathbb{E}_{\tau \sim \pi'}\left[\sum_{t=0}^\infty \gamma^t (Q_\pi(s_t,a_t) - V_\pi(s_t))\right] \quad \text{(By definition of $A_\pi$)} \\
...
\paragraph{b) Proof of Equation (\ref{true_objective})}
Using the definition of $\rho_{\pi'}(s)$:
\begin{align*}
\eta(\pi') - \eta(\pi) &= \sum_{t=0}^\infty \gamma^t \mathbb{E}_{s_t \sim \pi'}[\bar{A}(s_t)] \\
&= \sum_{s} \rho_{\pi'}(s) \sum_{a} \pi'(a|s) A_\pi(s,a)
\end{align*}
The equality follows because $\rho_{\pi'}(s) = \sum_{t=0}^\infty \gamma^t P(s_t = s|\pi')$. $\square$
...
\paragraph{c) Proof of Lemma 1} For $\alpha$-coupled policies:
\begin{align*}
|\bar{A}(s)| &= \left|\mathbb{E}_{a'\sim\pi'} A_\pi(s,a') - \mathbb{E}_{a\sim\pi} A_\pi(s,a)\right| \\
&\leq 2\alpha \max_{a} |A_\pi(s,a)| \quad \text{(Since $P(a\neq a') \leq \alpha$)}
\end{align*}
Taking max over $s,a$ gives the result. $\square$
...
\paragraph{d) Proof of Lemma 2} Let $\delta_t = \mathbb{E}_{s_t \sim \pi'}[\bar{A}(s_t)] - \mathbb{E}_{s_t \sim \pi}[\bar{A}(s_t)]$. Using coupling:
\begin{align*}
|\delta_t| &\leq 4\alpha(1-(1-\alpha)^t)\epsilon \quad \text{(By induction on $t$)} \\
\text{Base case } t=0: & \delta_0 = 0 \\
\text{Inductive step: } & \delta_{t+1} \leq \gamma(1-\alpha)\delta_t + 2\alpha\gamma\epsilon
\end{align*}
Solving the recurrence gives the bound. $\square$
...
\paragraph{e) Proof of Lemma 3} Summing over time horizons:
\begin{align*}
|\eta(\pi') - L_\pi(\pi')| &= \left|\sum_{t=0}^\infty \gamma^t \delta_t\right| \\
&\leq \sum_{t=0}^\infty \gamma^t 4\alpha^2\epsilon(1-(1-\alpha)^t) \\
&= \frac{4\alpha^2\gamma\epsilon}{(1-\gamma)^2} \quad \text{(Geometric series sum)}
\end{align*}
$\square$
...
\paragraph{f) Proof of Theorem \ref{thm:policy-bound}} Using Pinsker's inequality $D_{TV} \leq \sqrt{D_{KL}/2}$:
\begin{align*}
\alpha &= D_{TV}^{\max}(\pi,\pi') \leq \sqrt{D_{KL}^{\max}(\pi,\pi')/2} \\
|\eta(\pi') - L_\pi(\pi')| &\leq \frac{4(\sqrt{D_{KL}/2})^2\gamma\epsilon}{(1-\gamma)^2} = \frac{2D_{KL}\gamma\epsilon}{(1-\gamma)^2}
\end{align*}
Strengthening the bound by factors of 2 gives the theorem. $\square$
